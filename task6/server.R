...
shinyServer(function(input, output) {
    ...
    model1 <- reactive({
        ...
        #vibor
        w <- rep(NA, 6)
        p_goods <- as.numeric(input$p_goods)
        w[1] <- as.numeric(input$w1)
        w[2] <- as.numeric(input$w2)
        w[3] <- as.numeric(input$w3)
        w[4] <- as.numeric(input$w4)
        ...
        t_oborot <- rep(NA,4)
        t_oborot[1] <- as.numeric(input$t_oborot1)
        t_oborot[2] <- as.numeric(input$t_oborot2)
        t_oborot[3] <- as.numeric(input$t_oborot3)
        t_oborot[4] <- as.numeric(input$t_oborot4)
        
        check_t = as.numeric(input$check_t)
        check_kartel = as.numeric(input$check_kartel)
        check_zao = as.numeric(input$check_zao)
        
        
        if (check_zao == 1) {
            mrc = as.numeric(input$mrc)/100 # vvod danyh
        } else {
            mrc = 1
        }
    ...
    output$text1 <- renderText({ 
            tryCatch({
                ...
                print(paste("Labor: monopoly ", res_shiny[1], ", firm 1 - ", res_shiny[2], ", firm 2 - ", res_shiny[3],
                            ", firm 3 - ", res_shiny[4], ", government - ", res_shiny[5], ", unemployment - ", res_shiny[6], ". Budget: ", round(res_shiny[7]), ".", sep = ""))

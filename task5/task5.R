...
#lider stradegy
check_x_mark = 1 #chto delat esli pribil padayet
check_cost_mark = 5*10^6
check_x_factory = 1 #checkbox 0/1
check_n_x_new_a = 2
check_n_x_new_b = 2
check_x_price = 0 #0.1
check_x_price_incr <- 0.1

#other control parametrs
x_p_a <- 300 #price 1 chorhih
x_p_b <- 350 #price 1 roz
...
x_inf <- 0.06
x_a_1z <- 17*10^6/12 #kilkist chornih gadget from 1 zavod
x_b_1z <- 16*10^6/12 #k-st rozevih gadget from 1 zavod
...
#china
y_inf <- 0.046
y_1z <- 10*10^6/12 #kilkist gadget from 1 zavod
...
g1 + geom_line() + 
    ylim(0,100) + 
    labs(x = "Period",y = "Share of leader, %") +
...
Q = q_y+q_x_a+q_x_b
plot((q_x_a+q_x_b)/Q, type = "l", ylim = c(0,1))
lines((q_y)/Q, type = "l", col = "red")
